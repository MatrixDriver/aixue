...
2. **严格判断每道题是否完整**，以下任何一种情况都算**不完整**（complete=false）:
   - 题目开头被截断（只看到题目的后半部分，缺少题目的起始描述或已知条件）
   - 题目末尾被截断（问句不完整，缺少"求…"、"计算…"等关键问题部分）
   - 题目的关键条件或数据缺失（如图片边缘切掉了部分文字）
   - 只能看到题号但看不到完整题干
   - 需要依赖图片中未显示的上文信息才能理解题意
3. 用一句话概括每道题的内容（不超过20字）
4. 最多识别5道题

输出格式（严格 JSON，不要包含其他文字）:
{"question_count": 数字, "questions": [{"index": 1, "label": "题号文字", "summary": "一句话概括", "complete": true或false}]}
