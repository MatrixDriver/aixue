...
MULTI_QUESTION_DETECTION_PROMPT = """请分析这张图片中包含的**独立大题**，输出 JSON 格式结果。

关键规则（必须严格遵守）:
1. **只识别大题**，题号通常为阿拉伯数字（如"13."、"14."、"第1题"等）
2. **小问不是独立题目**：(1)(2)(3)、①②③、a.b.c. 等是大题下的小问，必须归属到其所在的大题，不要单独列出
3. **严格判断完整性**，以下任何一种情况都算不完整（complete=false）:
   - 题目开头被截断（缺少起始描述或已知条件）
   - 题目末尾被截断（问句不完整）
   - 关键条件或数据被图片边缘切掉
   - 只能看到题号但题干不完整
   - 需要依赖图片外的上下文才能理解题意
4. 用一句话概括每道题的内容（不超过20字）
5. 最多识别5道大题

输出格式（严格 JSON，不要包含其他文字）:
{"question_count": 数字, "questions": [{"index": 1, "label": "题号", "summary": "一句话概括", "complete": true或false}]}

示例: 图片中有第13题（不完整）和第14题（含(1)(2)两小问），应输出:
{"question_count": 2, "questions": [{"index": 1, "label": "13", "summary": "...", "complete": false}, {"index": 2, "label": "14", "summary": "...", "complete": true}]}
